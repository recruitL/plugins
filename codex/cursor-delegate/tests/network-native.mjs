...
const [parent,codexPath,mode="limited"]=process.argv.slice(2);
if (!["limited","full"].includes(mode)) throw new Error("Expected limited or full network mode");
...
async function request(url,method="GET"){if(!proxy)return {status:0}; const p=new URL(proxy);return await new Promise(ok=>{const r=http.request({method,hostname:p.hostname,port:p.port,path:url,agent:new http.Agent(),headers:{Host:new URL(url).host}},res=>{let body='';res.on('data',b=>{body+=b});res.on('end',()=>ok({status:res.statusCode,canary:body==='LOCAL_CANARY',diagnostic:res.statusCode===200?null:body.slice(0,600)}))});r.setTimeout(3000,()=>r.destroy());r.on('error',e=>ok({status:0,error:e.code}));r.end()})}
const a=await request('http://127.0.0.1:'+port+'/canary');checks.allowed_origin=a.status===200&&a.canary;
const post=await request('http://127.0.0.1:'+port+'/canary','POST');checks.method_policy_correct=process.argv[3]==='full'?post.status===200&&post.canary:post.status===403;
...
console.log(JSON.stringify({checks,allowed:a,post,denied:d,explicit}));process.exit(Object.values(checks).every(Boolean)?0:1);`;
const childPath=join(workspace,'probe.mjs');writeFileSync(childPath,code);
const child=spawn(realpathSync(codexPath),['sandbox','-P','probe','-C',workspace,'--include-managed-config','--',realpathSync(process.execPath),childPath,String(originPort),mode],{cwd:workspace,detached:true,env:{HOME:base,CODEX_HOME:config,PATH:'/usr/bin:/bin',TMPDIR:workspace},stdio:['ignore','pipe','pipe']});
...
const receipt={kind:'native Codex managed network proxy; localhost canary only; no Cursor or credentials',base,mode,...result,timedOut,requests,observed,stderr};
writeFileSync(join(base,'receipt.json'),JSON.stringify(receipt,null,2));console.log(JSON.stringify(receipt,null,2));
process.exitCode=result.code===0&&requests===(mode==="full"?2:1)?0:1;
